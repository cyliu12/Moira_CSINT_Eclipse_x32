...
//import org.athomeprojects.moira.AppIO;
...
public class Moira {
	...
	public static MRI mri = new MRI(); // added by cyliu

	public static void init() {
		io = new AppIO(null);
		FileIO.setBaseIO(io);
		new Resource(Moira.class.getClass(), null, null, null, null);	 	
	...
	public static void disposeMoira() { // added by cyliu below
		chart_tab.dispose();
		table_tab.dispose();
	...
	static public void update(boolean delay_update, boolean diagram_on_top) {
		...
		if (delay_update) {
			...
			// checkpoint:Pass data to csharp here

			/*
			 * InjLocationSpinner loc = chart_tab.getSpinner(); ChartData data =
			 * chart_tab.getData(); int[] birth_date = new int[5]; int[]
			 * now_date = new int[5]; chart_tab.getBirthDate(birth_date);
			 * Log.c("Name : " + chart_tab.getName()); Log.c("Sex : " +
			 * chart_tab.getSex()); Log.c("Country/City/Zone : " +
			 * loc.getCountryName() + " , " + loc.getCityName() + " , " +
			 * loc.getZoneName()); Log.c("Birth : " + birth_date[0] + "/" +
			 * birth_date[1] + "/" + birth_date[2] + "  " + birth_date[3] + ":"
			 * + birth_date[4]); Log.c("Now : " + now_date[0] + "/" +
			 * now_date[1] + "/" + now_date[2] + "  " + now_date[3] + ":" +
			 * now_date[4]);
			 */
	...
	static private void initProgressBar() {
		...
		progress_indicator.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		...
		if (Resource.trace) {
			...
			try {
				...
				if (!after_bound.equals(before_bound)) {
					adjust_position = new Rectangle(before_bound.x - after_bound.x, before_bound.y - after_bound.y,
							before_bound.width - after_bound.width, before_bound.height - after_bound.height);
				}
	...
	/*
	 * static public void main(String[] args) { final String install_path, file;
	 * if (args.length == 1 || args.length == 2) { install_path = args[0]; file
	 * = (args.length == 2) ? args[1] : null; } else { install_path = file =
	 * null; } display = new Display(); shell = new Shell(display);
	 * shell.setBackground(display.getSystemColor(SWT.COLOR_WHITE)); final
	 * StackLayout layout = new StackLayout(); layout.marginWidth =
	 * layout.marginHeight = 0; shell.setLayout(layout); io = new
	 * AppIO(install_path); FileIO.setBaseIO(io); initProgressBar();
	 * display.asyncExec(new Runnable() { public void run() { new Moira(shell,
	 * install_path, file); layout.topControl = tab_folder; setShellTitle(null,
	 * null, false, false); setShellPosition(null, null); setShellState();
	 * chart_tab.setSashWeights(); chart_tab.layout(true);
	 * shell.addShellListener(new ShellListener() { public void
	 * shellClosed(ShellEvent e) { ChartTab.hideTip(); if (!closing &&
	 * !table_tab.checkForSave()) { e.doit = false; return; } active = false;
	 * shut_down = true; exit(false); }
	 * 
	 * public void shellActivated(ShellEvent e) { active = true; }
	 * 
	 * public void shellDeactivated(ShellEvent e) { active = false; }
	 * 
	 * public void shellIconified(ShellEvent e) { if
	 * (Resource.getPrefInt("tray_icon") == 0) return; ChartTab.hideTip();
	 * Shell[] shells = display.getShells(); for (int i = 0; i < shells.length;
	 * i++) { if (shells[i] != shell && shells[i].getParent() == null &&
	 * !ChartTab.isTipShell(shells[i])) shells[i].setVisible(false); }
	 * shell.setVisible(false); }
	 * 
	 * public void shellDeiconified(ShellEvent e) { if
	 * (Resource.getPrefInt("tray_icon") == 0) return; ChartTab.hideTip();
	 * shell.setVisible(true); Shell[] shells = display.getShells(); for (int i
	 * = 0; i < shells.length; i++) { if (shells[i] != shell &&
	 * shells[i].getParent() == null && !ChartTab.isTipShell(shells[i]))
	 * shells[i].setVisible(true); } } }); shell.addListener(SWT.Resize, new
	 * Listener() { public void handleEvent(Event event) { if
	 * (shell.getMaximized() || shell.getMinimized()) return; resizing++;
	 * display.timerExec(RESIZE_DELAY, new Runnable() { public void run() { if
	 * (shell.isDisposed()) return; if (--resizing == 0) { if
	 * (!shell.getMaximized() && !shell.getMinimized()) auditShellPosition();
	 * Canvas canvas = chart_tab.getDiagram(); canvas.getParent().layout();
	 * canvas.redraw(); } } }); } }); shell.addKeyListener(new KeyAdapter() {
	 * public void keyReleased(KeyEvent event) { if (event.keyCode == SWT.F5 ||
	 * event.keyCode == SWT.F6) { FolderToolBar .findNextEntry(event.keyCode ==
	 * SWT.F5); } } }); openShell(); } }); flushEvents(false); endProgress();
	 * flushEvents(true); }
	 */
