...
public class ChartData {
    ...
    private String[] stellar_names = new String[28];	//28宿
    ...
    public void loadResource()
    {
        ...
        birth_year_info = Resource.getString("birth_year_info");	//天祿等值年諸神(七政四餘)
        current_year_info = Resource.getString("current_year_info");
        pick_year_info = Resource.getString("pick_year_info");
        alt_birth_year_info = Resource.getString("alt_birth_year_info");	//比肩等值年諸神(子平八字)
        ...
        Resource.getStringArray("birth_year_signs", year_signs);	//命宮, 相貌, 福德...12宮逆數
        Resource.getStringArray("astro_cusps_name", cusps_name);	//命宮, 財帛, 兄弟...12宮順數
        ...
        Resource.getStringArray("full_twelve_signs", full_twelve_signs);	//金兌酉, 水坤申, ...
        Resource.getStringArray("chinese_zodiac_signs", chinese_zodiac_signs);	//雞, 猴, 羊, 馬, 蛇, 龍, 兔, 虎, 牛, 鼠, 豬, 狗
        Resource.getStringArray("birth_year_names", year_names);	//甲子, 乙丑, 丙寅, ... 60甲子
        ...
        Resource.getStringArray("earth_god_seq", earth_god_seq);	//子:癸, 丑:癸辛己, 寅:甲丙戊, 卯:乙, 辰:戊乙癸, 巳:庚丙戊, 午:丁己, 未:乙己丁, 申:庚壬戊, 酉:辛, 戌:辛丁戊, 亥:壬甲
        ...
        Resource.getStringArray("season_starts", season_starts);	//小寒, 大寒, 立春, ... 24節氣
        Resource.getStringArray("full_zodiac", full_zodiac);	//戌火, 酉金, 申水, 未月, 午日, 巳水, 辰金, 卯火, 寅木, 丑土, 子土, 亥木
        Resource.getStringArray("full_stellar_signs", full_stellar_signs);	//婁金, 胃土, 昴日, ... 28宿
        Resource.getStringArray("half_stellar_signs", half_stellar_signs);	//婁, 胃, 昴, ... 28宿
        Resource.getStringArray("stellar_names", stellar_names);	//beAri, ta-6Eri, 16Tau, 28宿
        ...
        Resource.getStringArray("compass_stellar_names", compass_stellar_names);	//16.0, 29.0, 44.0, 28宿星盤刻度
        Resource.getStringArray("mountain_signs", mountain_signs);	//辛山, 酉山, 庚山, ... 24山
        Resource.getStringArray("full_mountain_main_signs",			//辛金, 酉金, 庚金, ... 24山
                full_mountain_main_signs);
        Resource.getStringArray("full_mountain_secondary_signs",	//辛火, 酉金, 庚金, ... 24山
        ...
        fixstar_signs = Resource.getStringArray("fixstar_signs");	//天皇,  天帝,   尊帝,   ...
            ...
            if (birth_data) {
                ...
                if (!ChartMode.isChartMode(ChartMode.ASTRO_MODE)) {
                    ...
                    String[] array = (ChartMode
                            ...
                            : stellar_names;	//sidereal_stellar_names => 直接指定度數，不含歲差修正 //stellar_names是現代天文學星宿名稱，計算其位置就有歲差修正 
                    double offset = 0.0;
                    if (house_mode) {
                        if (Resource
                                .getPrefInt(ChartMode
    ...
    private String getGauquelin(int planet_no) //cyliu:Gauquelin = 相位圖
    {
        if (!show_gauquelin)
            return "";
    ...
    public void copyToMRI(){
		...
		Moira.mri.result.birth_poles = birth_poles;
		...
		Moira.mri.result.compass_sign_pos = compass_sign_pos;	//開禧宿度
		...
		Moira.mri.result.now_poles = now_poles;
		...
		Moira.mri.result.now_sign_pos = now_sign_pos;
